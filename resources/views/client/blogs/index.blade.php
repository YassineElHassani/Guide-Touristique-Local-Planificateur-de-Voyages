        ...
        <div class="col-lg-3">
            <div class="dashboard-sidebar p-4">
                ...
                <div class="mb-4">
                    ...
                    <a href="{{ route('client.blogs.index') }}" class="sidebar-link {{ request()->routeIs('client.blogs.index') ? 'active' : '' }}">
                    <i class="fas fa-blog"></i> All Blogs
                    </a>
                
                <a href="{{ route('client.blogs.my-blogs') }}" class="sidebar-link {{ request()->routeIs('client.blogs.my-blogs') ? 'active' : '' }}">
                ...
                <div class="mb-4">
                    ...
                    <div class="list-group">
                        <a href="{{ route('client.blogs.index') }}" class="list-group-item list-group-item-action {{ request()->routeIs('client.blogs.index') && !request()->query('category') ? 'active' : '' }}">
                            All Categories
                        </a>
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-md-5 mt-3 mt-md-0">
                        <div class="d-flex justify-content-md-end">
                            <form action="{{ route('client.blogs.index') }}" method="GET" class="d-flex me-2 flex-grow-1">
                        ...
                        @foreach($featuredBlogs as $index => $featuredBlog)
                            <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                                <div class="position-relative">
                                    ...
                                    <div class="carousel-caption text-start" style="background: rgba(0,0,0,0.6); left: 0; right: 0; bottom: 0; padding: 20px;">
                                        <span class="badge bg-warning mb-2">Featured</span>
                                        <h3>{{ $featuredBlog->title }}</h3>
                                        <p>{{ Str::limit($featuredBlog->excerpt, 120) }}</p>
                                        <a href="{{ route('client.blogs.show', $featuredBlog->slug) }}" class="btn btn-primary btn-sm">Read More</a>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div class="card blog-card h-100">
                                <div class="position-relative">
                                    @if($blog->image)
                                        <img src="{{ asset('storage/' . $blog->image) }}" 
                                             ...
                                             alt="{{ $blog->title }}">
                                    @else
                                        <div class="bg-light d-flex align-items-center justify-content-center blog-image">
                                ...
                                <div class="card-body d-flex flex-column">
                                    ...
                                    <div class="text-muted small mb-3">
                                        ...
                                        <span class="ms-3"><i class="far fa-comments me-1"></i> {{ $blog->comments_count }}</span>
                                        <span class="ms-3"><i class="far fa-eye me-1"></i> {{ $blog->views }}</span>
                                    </div>
                                    
                                    <p class="card-text flex-grow-1">{{ Str::limit($blog->excerpt, 100) }}</p>
                                    
                                    <div class="mt-auto d-flex justify-content-between align-items-center">
                                        <a href="{{ route('client.blogs.show', $blog->slug) }}" class="btn btn-sm btn-primary">
                                            Read More
                                        </a>
                                        ...
                                        @if(Auth::id() === $blog->user_id)
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    <li>
                                                        <a class="dropdown-item" href="{{ route('client.blogs.edit', $blog->slug) }}">
                                                            <i class="fas fa-edit me-1 text-success"></i> Edit
                                                        </a>
                                                    </li>
                                                    <li>
                                                        <form action="{{ route('client.blogs.update-status', $blog->id) }}" method="POST">
                                                            @csrf
                                                            @method('PATCH')
                                                            <input type="hidden" name="published" value="{{ $blog->published ? '0' : '1' }}">
                                                            <button type="submit" class="dropdown-item">
                                                                @if($blog->published)
                                                                    <i class="fas fa-eye-slash me-1 text-warning"></i> Unpublish
                                                                @else
                                ...
                                <div class="card-footer bg-white">
                                    <div class="d-flex align-items-center">
                                        @php
                                            $avatar = $blog->user && $blog->user->picture
                                                ? (Str::startsWith($blog->user->picture, 'http')
                                                    ? $blog->user->picture
                                                    : asset('storage/' . $blog->user->picture))
                                                : asset('/assets/images/default-avatar.png');
                                        @endphp
                                        <img src="{{ $avatar }}" alt="{{ $blog->user ? $blog->user->first_name : 'User' }}" 
