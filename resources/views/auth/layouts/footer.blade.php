<footer class="footer footer-style1">
    <div class="tf-container">
        <div class="footer-main">
            <div class="footer-logo">
                <div class="logo-footer">
                    <img src="{{ url('/assets/images/logo2.png') }}" alt="">
                </div>
                <p class="des-footer">The world’s first and largest digital market
                    for crypto collectibles and non-fungible
                </p>
                <ul class="footer-info">
                    <li class="flex-three">
                        <i class="icon-noun-mail-5780740-1"></i>
                        <p>Info@webmail.com</p>
                    </li>
                    <li class="flex-three">
                        <i class="icon-Group-9"></i>
                        <p>684 555-0102 490</p>
                    </li>
                    <li class="flex-three">
                        <i class="icon-Layer-19"></i>
                        <p>6391 Elgin St. Celina, NYC 10299</p>
                    </li>
                </ul>

            </div>
            <div class="footer-service">
                <h5 class="title">Services Req</h5>

                <ul class="footer-menu">
                    <li>
                        <a href="about-us.html">About Us</a>
                    </li>
                    <li>
                        <a href="gallery.html">Gallery</a>
                    </li>
                    <li>
                        <a href="team.html">Our Team</a>
                    </li>
                    <li>
                        <a href="blog.html">Blog Insights</a>
                    </li>
                    <li>
                        <a href="contact-us.html">Contact</a>
                    </li>
                </ul>

            </div>
            <div class="footer-gallery">
                <h5 class="title">Gallery</h5>

                <div class="gallery-img">
                    <a href="{{ url('/assets/images/gallery/gl1.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl1.jpg') }}" alt="image gallery">
                    </a>
                    <a href="{{ url('/assets/images/gallery/gl2.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl2.jpg') }}" alt="image gallery">
                    </a>
                    <a href="{{ url('/assets/images/gallery/gl3.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl3.jpg') }}" alt="image gallery">
                    </a>
                    <a href="{{ url('/assets/images/gallery/gl4.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl4.jpg') }}" alt="image gallery">
                    </a>
                    <a href="{{ url('/assets/images/gallery/gl5.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl5.jpg') }}" alt="image gallery">
                    </a>
                    <a href="{{ url('/assets/images/gallery/gl6.jpg') }}" data-fancybox="gallery">
                        <img src="{{ url('/assets/images/gallery/gl6.jpg') }}" alt="image gallery">
                    </a>
                </div>

            </div>
            <div class="footer-newsletter">
                <h5 class="title">Newsletter</h5>
                <form action="/" id="footer-form">
                    <div class="input-wrap flex-three">
                        <input type="email" placeholder="Enter Email Adress">
                        <button type="submit"><i class="icon-paper-plane"></i></button>
                    </div>
                    <div class="check-form flex-three">
                        <i class="icon-Vector-121"></i>
                        <p>I agree to all your terms and policies</p>
                    </div>

                </form>
                <ul class="social-ft flex-three">
                    <li>
                        <a href="#">
                            <i class="icon-icon-2"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-x"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-8"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-2"></i>
                        </a>
                    </li>
                </ul>

            </div>
        </div>

        <div class="row footer-bottom">
            <div class="col-md-6">
                <p class="copy-right">Copyright © 2024 by Themesflat. All Rights Reserved</p>
            </div>
            <div class="col-md-6">
                <ul class="social flex-six">
                    <li>
                        <a href="#">
                            <i class="icon-icon-2"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-x"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-icon_03"></i>
                        </a>
                    </li>
                    <li>
                        <a href="#">
                            <i class="icon-2"></i>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
    </div>
</footer>

<!-- Bottom -->
</div>
<!-- /#page -->
</div>

<!-- Modal Popup Bid -->

<a id="scroll-top" class="button-go"></a>

<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" aria-labelledby="offcanvasRightLabel">
    <div class="offcanvas-header">
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body">
        <div class="logo-canvas">
            <img src="{{ url('/assets/images/logo.png') }}" alt="image">
        </div>
        <p class="des">The world’s first and largest digital market
            for crypto collectibles and non-fungible
        </p>
        <ul class="canvas-info">
            <li class="flex-three">
                <i class="icon-noun-mail-5780740-1"></i>
                <p>Info@webmail.com</p>
            </li>
            <li class="flex-three">
                <i class="icon-Group-9"></i>
                <p>684 555-0102 490</p>
            </li>
            <li class="flex-three">
                <i class="icon-Layer-19"></i>
                <p>6391 Elgin St. Celina, NYC 10299</p>
            </li>
        </ul>
        <ul class="social flex-three">
            <li>
                <a href="#">
                    <i class="icon-icon-2"></i>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="icon-x"></i>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="icon-8"></i>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="icon-6"></i>
                </a>
            </li>
        </ul>

    </div>
</div>

<!-- Javascript -->
<script src="{{ url('/app/js/jquery.min.js') }}"></script>
<script src="{{ url('/app/js/jquery.nice-select.min.js') }}"></script>
<script src="{{ url('/app/js/bootstrap.min.js') }}"></script>
<script src="{{ url('/app/js/swiper-bundle.min.js') }}"></script>
<script src="{{ url('/app/js/swiper.js') }}"></script>
<script src="{{ url('/app/js/plugin.js') }}"></script>
<script src="{{ url('/app/js/jquery.fancybox.js') }}"></script>
<script src="{{ url('/app/js/shortcodes.js') }}"></script>
<script src="{{ url('/app/js/main.js') }}"></script>

</body>

</html>
